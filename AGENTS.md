...
## 项目结构与模块组织
- `packages/cli`: CLI 入口 `src/cli.ts`，构建产物在 `dist/`，并通过 `scripts/build-web.ts` 打包前后端资源。
...
- 根目录通过 `package.json` 的 `workspaces` 管理 Bun workspace，并包含通用 TS 配置。
...
## 编码风格与命名规范
- 统一 TypeScript + ESM；运行时和包管理器使用 Bun。
